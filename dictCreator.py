...
filenameUS = open("enUS.txt","r")
filenameGB = open("enGB.txt","r")
outfileUS = open("metaphonicDictUSsec.txt","w")
outfileGB = open("metaphonicDictGBsec.txt","w")

for line in filenameUS:
    if line.find("/") == -1:
        if line[-1] == "\n":
            outfileUS.write(dm(line[:-1])[1]+"\n")
        else:
            outfileUS.write(dm(line)[1]+"\n")
    else:
        outfileUS.write(dm(line[:line.find("/")])[1]+"\n")
        
filenameUS.close()
outfileUS.close()
        
for line in filenameGB:
    if line.find("/") == -1:
        if line[-1] == "\n":
            outfileGB.write(dm(line[:-1])[1]+"\n")
        else:
            outfileGB.write(dm(line)[1]+"\n")
    else:
        outfileGB.write(dm(line[:line.find("/")])[1]+"\n")
